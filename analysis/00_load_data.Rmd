...
gt <- as.matrix(dat)
miss_smpl <- apply(gt, 1, function(x) {sum(is.na(x))/nrow(gt)})
na_vars <- apply(gt, 2, function(x) {sum(is.na(x))/nrow(gt)})
nonmiss_vars <- which(log2(na_vars + 1) < 0.001) %>% as.integer()
nonmiss_idx <- which(miss_smpl < 0.1) %>% as.integer()
...
gt <- as.matrix(dat)
gt <- gt[nonmiss_idx,nonmiss_vars]

nonzero_var <- which(apply(gt, 2, function(x) {var(x, na.rm = T)}) != 0)
gt_std <- apply(gt, 2, function(x) {(x - mean(x, na.rm = T)) / sd(x, na.rm = T)})
gt_std <- gt_std[, nonzero_var]
```

```{r}
gt_pca <- prcomp(t(gt_std), center = F, scale. = F)
...
ggplot2::ggplot(pca_df, ggplot2::aes(x = pc, y = cumsum(pve)/100)) + ggplot2::geom_point(stat = "identity") +
  xlim(0, 20)

evec <- data.frame(gt_pca$rotation[,seq(10)]) %>%
